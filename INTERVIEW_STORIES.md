...
### P5：本地模型支持 + 多模型切换 —— 从单一 API 到多 Provider 路由

**问题**

"系统原来只支持一个 LLM 提供商，通过环境变量 `LLM_API_KEY` + `LLM_BASE_URL` + `LLM_MODEL` 配置。这有几个问题：

1. 开发调试时每次调用都消耗 API 额度，成本高
2. 无法使用本地模型（如 Ollama），数据必须发送到云端
3. 用户无法在前端切换模型，必须改环境变量重启服务
4. 如果 OpenAI API 出问题，整个系统不可用"

**动作**

"我做了四层改造：

1. **多客户端缓存**：按 `(base_url, api_key)` 缓存 `AsyncOpenAI` 实例，不同 provider 用不同客户端
2. **模型注册表**：从环境变量自动检测可用模型（OpenAI、DeepSeek、Ollama），也支持 `MODEL_REGISTRY` JSON 自定义
3. **JSON mode 降级**：Ollama 等本地模型不支持 `response_format={'type': 'json_object'}`，自动跳过，靠 prompt 引导 + `json_repair` 兜底
4. **前端模型选择器**：`GET /api/models` 返回可用模型列表，前端下拉选择，localStorage 持久化

关键设计：`structured_completion()` 是所有 LLM 调用的唯一入口，新增 `model_id` 参数。内部调用 `resolve_model(model_id)` 返回 `(client, model_name, supports_json_mode)` 三元组。不传 `model_id` 时走原有默认路径，完全向后兼容。"

```python
# backend/utils/llm_client.py - resolve_model 核心逻辑
def resolve_model(model_id: str | None = None) -> tuple[AsyncOpenAI, str, bool]:
    ...
    return get_client(), get_model(), True  # 默认路径
```

**结果**

- 支持 OpenAI、DeepSeek、Ollama 三种 provider，可扩展
- 本地模型零 API 成本，数据不出本地
- 前端一键切换模型，无需重启服务
- 18 个测试覆盖 provider 检测、注册表构建、模型解析、客户端缓存

**数据**

- `_detect_provider_from_url()`: 5 种 URL 模式识别（`llm_client.py` 第 65-73 行）
- `_build_default_registry()`: 自动检测 3 种 provider（`llm_client.py` 第 76-130 行）
- 客户端缓存命中率：同 provider 100%（`_client_cache` 按 tuple key 去重）
- 测试覆盖：`test_model_routing.py` 有 18 个测试用例

**复盘**

"关键权衡：为什么不用 LiteLLM？

LiteLLM 是一个流行的多 provider 统一接口库，但对我们的场景来说太重了：
1. 我们只需要 2-3 个 provider，不需要 100+ provider 的路由表
2. 我们已经有 `AsyncOpenAI` 客户端和完整的重试逻辑
3. LiteLLM 引入额外依赖和运行时复杂度
4. 直接用 `AsyncOpenAI` + 不同 `base_url` 就能覆盖所有 OpenAI-compatible provider

所以选择了最简方案：多客户端缓存 + 模型注册表。如果未来 provider 数量增长到 5+，再考虑迁移到 LiteLLM。"

---

### P6：AI Runtime Layer Phase 1 —— Task-aware 路由 + Fallback + 模型能力检测

**问题**

"P5 实现了多模型切换，但所有任务用同一个模型。Planner 需要强推理能力（适合 GPT-4o），Extractor 需要快速结构化输出（适合 GPT-4o-mini），Writer 需要创造力（适合 Claude）。用同一个模型意味着要么成本高（全用 GPT-4o），要么质量差（全用 mini）。"

**动作**

"参考 OpenHands 应用级架构和 LiteLLM Router 模式，设计了三层抽象：

1. **TaskType + TaskRequirement**：5 种任务类型（planning/extraction/writing/qa/reflection），每种有硬约束（needs_structured_output、needs_long_context、max_cost_tier）和软偏好（prefers_creativity、latency_sensitive）
2. **ModelConfig 能力扩展**：新增 6 个字段（max_context_tokens、supports_long_context、cost_tier、reasoning_score、creativity_score、latency_score），按 provider/model 自动推断
3. **两阶段路由**：硬约束过滤（排除不满足条件的模型）→ 软偏好评分（加权排序选最优）

关键设计：`structured_completion()` 新增 `task_type` 参数，不传时走原有默认路径，完全向后兼容。"

```python
# backend/llm/router.py - 两阶段路由核心
def select_model(task_type, available_models, override_model_id=None):
    # Phase 1: 硬约束过滤
    for model in available_models.values():
        ...
        if not _cost_allowed(model.cost_tier, req.max_cost_tier):
            continue  # QA 任务排除 HIGH 成本模型
    # Phase 2: 软偏好评分
    score += model.reasoning_score * 2.0  # planning 权重高
    score += model.creativity_score * 1.5  # writing 权重高
    score += model.latency_score * 1.5    # extraction 权重高
    score += (4 - int(model.cost_tier)) * 0.8  # 便宜模型加分
```

**结果**

- 5 种任务类型 × 多模型自动路由
- Fallback 链：主模型失败时按评分顺序尝试下一个
- 43 个新测试全部通过，269 个总测试无回归
- 3 个新文件（`backend/llm/`），4 个修改文件，完全向后兼容

**复盘**

"关键权衡：为什么不用 LiteLLM Router？
1. LiteLLM 是 proxy-first 设计，我们是 application-first
2. 我们的路由逻辑是 task-aware 的（按任务类型选模型），LiteLLM 是 model-group-aware 的（按模型组负载均衡）
3. 自建路由 ~100 行代码，LiteLLM 引入 50+ 依赖
4. 如果未来需要更复杂的路由（如基于输入内容动态选模型），自建更容易扩展"

---

### P7：AI Runtime Layer Phase 2 —— YAML 配置 + 流式输出 + 成本追踪前端化

**问题**

"Phase 1 实现了 task-aware 路由，但模型配置硬编码在 Python 字典里。每次加模型要改代码、重新部署。另外 Writer 生成综述时用户要等 30-60 秒才能看到结果，没有中间反馈。成本追踪只在后端日志里，用户看不到。"

**动作**

"三个方向并行推进：

1. **YAML 配置加载器**：`config_loader.py` 从 `models.yaml` 加载模型配置，支持 `${VAR:-default}` 环境变量替换。无 YAML 文件时回退到内置默认配置，零破坏性。

2. **ContextVar 流式回调**：核心设计决策是用 `ContextVar` 而非修改函数签名。`structured_completion()` 有 5 层调用链（page.tsx → main.py → workflow → nodes → llm_client），改签名要动所有层。ContextVar 让 main.py 设置回调、llm_client 读取回调，中间层完全无感知。

3. **前端成本追踪**：`createSSEConnection` 从位置参数重构为 `SSECallbacks` 对象，新增 `onCostUpdate` / `onDraftToken` 回调。Zustand store 新增 `totalCostUsd`，status-indicator 实时显示 `$0.xxxx`。"

```python
# backend/utils/llm_client.py - ContextVar 流式回调
...
async def structured_completion(...):
    ...
    if on_token is not None:
        raw_content = await _call_llm_streaming(client, messages, on_token=on_token, ...)
    else:
        raw_content = await _call_llm(client, messages, ...)
```

```python
# backend/llm/config_loader.py - 环境变量替换
...
- YAML 配置：运维人员改 YAML 即可加模型，无需改代码
- 流式输出：用户在 Writer 生成时能看到逐 token 输出
- 成本追踪：前端实时显示运行成本，ModelSelector 显示 cost tier 标记
- 14 个新测试（config_loader 8 + streaming 6），全部通过

**复盘**

"ContextVar 的选择是这次最关键的设计决策。替代方案有三个：
1. 改函数签名：5 层调用链全部要改，破坏性大
2. 全局变量：线程不安全，并发请求会互相干扰
3. ContextVar：asyncio 原生支持，每个协程独立上下文，零侵入

另一个权衡：YAML 配置为什么不用 JSON Schema 验证？因为 Pydantic 的 `model_validate()` 已经做了类型验证，再加 JSON Schema 是重复工作。YAML 解析失败时回退到内置默认配置，保证系统永远可用。"
...
| 多模型智能路由 | P6 Task-aware 路由 + Fallback | backend/llm/router.py |
...
- P5: 本地模型支持 + 多模型切换 → 第 401-459 行
- P6: AI Runtime Layer Phase 1 → 第 462-510 行
- P7: AI Runtime Layer Phase 2 → 第 512-560 行
