...
- **Phase 2（扩展）**：外部配置 YAML + 流式输出 + 成本追踪 ✅ **已实现**
...
- **Phase 2（扩展，~1 周）**：外部配置 YAML + 流式输出 + 成本追踪 ✅ **已实现**
...
**Phase 2 实现时间**：2026-02-26  
**状态**：Phase 1-2 已实现，Phase 3 待评审

### Phase 2 实现详情

**2.1 外部配置 YAML**（`backend/llm/config_loader.py`）

从 `models.yaml` 加载模型配置，替代硬编码的 `MODEL_REGISTRY`：
- `load_models_config()` 读取 YAML，支持环境变量替换（`${VAR}` / `${VAR:-default}`）
- `get_model_registry()` 优先加载 YAML，无文件时回退到内置默认配置
- 支持所有 `ModelConfig` 字段：provider、能力评分、cost_tier、fallback_for 等

**2.2 流式输出**（`backend/utils/llm_client.py`）

基于 `ContextVar` 的 token 流式回调，无需修改函数签名：
- `token_callback_var: ContextVar[TokenCallback | None]` 全局上下文变量
- `_call_llm_streaming()` 使用 OpenAI streaming API，逐 token 调用回调
- `structured_completion()` 自动检测 callback，有则走流式路径
- `main.py` 在 SSE producer 中设置 callback，发射 `draft_token` 事件

**2.3 成本追踪增强**（`backend/evaluation/cost_tracker.py`）

- `estimate_cost_usd()` 支持最长匹配模型定价（如 `gpt-4o-mini` 优先于 `gpt-4o`）
- SSE 发射 `cost_update` 事件，前端实时显示运行成本
- 前端 Zustand store 新增 `totalCostUsd` 状态，status-indicator 显示 `$0.xxxx`

**2.4 前端适配**

- `createSSEConnection` 重构为 `SSECallbacks` 对象模式，支持 `onCostUpdate` / `onDraftToken`
- `ModelSelector` 显示 cost tier 标记（$/$$/$$）和 fallback 指向
- `types/index.ts` 新增 `CostTier` 类型和 `cost_tier` / `fallback_for` 字段

**新增文件**：

| 文件 | 职责 |
|------|------|
| `backend/llm/config_loader.py` | YAML 配置加载 + 环境变量替换 |
| `models.yaml` | 模型配置文件（示例） |
| `tests/test_config_loader.py` | 配置加载器测试（8 个） |
| `tests/test_streaming.py` | 流式输出测试（6 个） |

**修改文件**：

| 文件 | 变更 |
|------|------|
| `backend/utils/llm_client.py` | `token_callback_var` + `_call_llm_streaming()` + `structured_completion` 流式分支 |
| `backend/evaluation/cost_tracker.py` | 最长匹配定价逻辑 |
| `backend/main.py` | SSE producer 设置 token callback，发射 `draft_token` 事件 |
| `frontend/src/types/index.ts` | `CostTier` + `ModelConfig` 扩展 |
| `frontend/src/store/research.ts` | `totalCostUsd` 状态 |
| `frontend/src/lib/api/client.ts` | `SSECallbacks` 对象模式 |
| `frontend/src/app/page.tsx` | 3 处 SSE 调用适配 |
| `frontend/src/components/model-selector.tsx` | cost tier + fallback 显示 |
| `frontend/src/components/console/status-indicator.tsx` | 运行成本显示 |
| `frontend/src/i18n/messages/{en,zh}.json` | 成本相关 i18n keys |

**测试覆盖**：14 个新测试（config_loader 8 + streaming 6），全部通过
